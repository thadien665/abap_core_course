CLASS zcl_01_learning_v24 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_01_learning_v24 IMPLEMENTATION.
...
out->write( data = airports name = `Internal Table AIRPORTS:` ).
...
out->write( data = airports_inline name = `Internal Table AIRPORTS_INLINE:` ).
...
out->write( data = names name = `ID and Name of Airlines and Airports:` ).
  ENDMETHOD.
ENDCLASS.
